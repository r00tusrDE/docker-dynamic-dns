...
Having gotten in to Docker in recent years, I wanted to migrate my existing NO-IP script to a Docker. I modified the script to accept environmental variables as input instead of passing them on the command line and also added three services for DynDNS, DuckDNS and STRATO in addition to NO-IP.
...
* **SERVICE**: The service you are using. Currently, the script is setup to use DuckDNS (duckdns), DynDNS (dyndns), STRATO (strato) and NO-IP (noip). Set the service to the value in parenthesis.
